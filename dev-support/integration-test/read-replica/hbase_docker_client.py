...
class HBaseDockerClient:
    ...
    def wait_for_hbase_ui(self):
        ...
        print(f"--- Waiting for HBase UI: {self._cluster_name} on {url} ---")
        last_exception = None
        for attempt in range(1, self._max_retries + 1):
            try:
                response = requests.get(url)
                if response.status_code == 200:
                    print(f"SUCCESS: {self._cluster_name} UI is up.")
                    return True
    ...
    def check_server_status(self):
        ...
        print(f"--- Validating Cluster Status: {self._cluster_name} ({self._container_name}) ---")
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if all(validations.values()):
                    for check, status in validations.items():
                        print(f"    [PASS] {check}")
                    print(f"SUCCESS: {self._cluster_name} is fully operational.")
                    return True
                else:
                    print(f"\nWARN: {self._cluster_name} is responding, but not all "
    ...
    def __run_hbase_command(self, hbase_cmd):
        ...
        print(f"Running command on {self._cluster_name}: {cmd_str}")
    ...
    def create_table(self, table_name, column_family):
        ...
        if f"Created table {table_name}" not in output:
            print(f"ERROR: Could not create table {table_name} on {self._cluster_name}")
            return False
        print(f"Created table {table_name} on {self._cluster_name}")
        return True
    ...
    def clean_up_tables(active_cluster, replica_cluster):
        ...
        if tables:
            print(f"Removing all existing tables on the {active_cluster.name}: {tables}")
            for table in tables:
                active_cluster.disable_table(table)
                active_cluster.drop_table(table)
            print(f"Running 'refresh_meta' on the {replica_cluster.name} cluster to sync it with "
...
if __name__ == "__main__":
    ...
    try:
        ...
        assert active_cluster.verify_table_exists(table_name), \
            f"Expected table '{table_name}' to exist on the {active_cluster.name}"
        assert not replica_cluster.verify_table_exists(table_name), \
            f"Table '{table_name}' should not exist on the {replica_cluster.name}"

        # Read-Replica cluster should now see the newly created table
        replica_cluster.refresh_meta()
        assert replica_cluster.verify_table_exists(table_name), \
            (f"Expected table '{table_name}' to exist on the {replica_cluster.name} "
        ...
        assert replica_cluster.verify_table_exists(table_name), \
            (f"Expected table '{table_name}' to still exist on the {replica_cluster.name} "
        ...
        assert not replica_cluster.verify_table_exists(table_name), \
            (f"Expected table '{table_name}' no longer exist on {replica_cluster.name} "
             f"after running 'refresh_meta'")
    except (RuntimeError, HBaseShellCommandError) as e:
        print(f"An error occurred:\n{e}")
        print("Cleaning up any tables that may be remaining")
        HBaseDockerClient.clean_up_tables(active_cluster, replica_cluster)
